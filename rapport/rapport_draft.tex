...
% ---------------------------------------------------------------------------
% Synthèse Kalman sur le dataset LEO labellisé (figures générées par le
% notebook src/maneuver_detection/explore_results.ipynb, section
% "Run des filtres avec paramètres optimaux / moyens / de clusters").
% NB : compiler après avoir exécuté le notebook pour que les figures
% kalman_*.png existent dans rapport/figures.
% ---------------------------------------------------------------------------
\subsubsection{Synthèse : filtre de Kalman sur l'ensemble du dataset LEO labellisé}

Nous synthétisons ici les résultats du filtre de Kalman sur l'ensemble des satellites LEO labellisés à la main (dataset Rivesaltes).
...
\begin{itemize}
    \item \textbf{fail} : $F1 < 0.5$
    \item \textbf{mid} : $0.5 \leq F1 < 0.7$
    \item \textbf{good} : $0.7 \leq F1 < 1$
    \item \textbf{perfect} : $F1 = 1$
\end{itemize}
Sur les 75 objets du dataset initial, 46 sont réellement exploitables (données SpaceTrack disponibles et manoeuvres labellisées).
...
% TODO : compléter avec les ratios exacts renvoyés par ratios() dans explore_results.ipynb
Seule une minorité de satellites reste en catégorie fail : leur pattern of life (bruit TLE, cadence de manoeuvres, densité de mesures) se prête mal à une détection par seuillage statistique de la NIS.
...
Le paramètre \texttt{min\_cluster\_size} de HDBSCAN est lui-même sélectionné par un scan : on retient la valeur qui maximise l'indice de validité DBCV tout en limitant la fraction de points classés en bruit (cluster $=-1$) :
\begin{center}
    \includegraphics[width=0.7\textwidth]{figures/kalman_hdbscan_selection.png}
\end{center}
...
\end{center}
% TODO : compléter avec les F1 moyennes par config (csv kalman_f1_comparison_configs.csv)

Trois enseignements se dégagent :
\begin{itemize}
    \item les paramètres optimaux par satellite constituent bien la borne supérieure, mais leur obtention nécessite des labels — la méthode ne passe pas à l'échelle du catalogue ;
    \item les paramètres moyens dégradent nettement la détection : la dispersion des paramètres optimaux observée sur le dataset (cf. \texttt{stats\_on\_params}) confirme qu'un réglage unique ne peut pas convenir à la diversité des pattern of life LEO ;
...
\subsubsection{A suivre 2 :}
\begin{itemize}
    \item Nous avons implémenté un algorithme déterministe qui nous permet d'avoir une baseline pour notre étude : le filtrage Kalman (d'ordre 1 ou 2 selon la finesse nécessaire). On a également la méthode utilisant la longitude moyenne qui est utilisable en GEO. 
    \item En GEO, nous avons maintenant bien définis les différents patter of life des satellites, grâce aux travail des chercheurs du MIT et du dataset SPLID
    \item En LEO, notre méthode est prometteuse en termes de précision mais trop grossière, on pourrait la pousser. Cependant, elle est surtout peu automatisable : elle nécessite toujorus in fine du cas par cas.
    \item En LEO, on va définir des sous-classes, pour pouvoir mieux distinguer les différents pattern of life des satllites.
