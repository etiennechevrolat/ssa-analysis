...
\subsection{Synthèse : filtre de Kalman sur un dataset LEO labellisé à la main}
...
Pour chaque satellite, les hyperparamètres du filtre $(var_Q, r, p_0)$ ainsi que l'ordre (1 ou 2) ont d'abord été optimisés individuellement, par maximisation de la F1 lissée (cf. sections précédentes), avec un seuil statistique $\chi^2_1$ fixe ($\alpha = 0.997$) et une tolérance d'association de 3 jours.
...
Une passe d'optimisation dure en moyenne entre 30s et 1min par objet (cela pourrait être diminué, notamment avec d'autres techniques d'optimisation, un paragraphe pour détailler cette optimsiation ?)
...
\end{center}
Ce premier exemple montre quelque chose de très particulier : lorsque les pics sont trop marqués/étalés dans le temps (ce qui correspond à des manoeuvres longues), le seuillage statistique (métrique[t] > seuil) crée de fausses manoeuvres. Dans cet exemple, la détection est en fait très bonne, il faut simplement un moyen plus intelligent de seuiller : le seuillage par proéminence topographique (cf. Zollo et al) 
...
\end{center}
Ici on a clairement une mauvaise détection des manoeuvres. La détection ne fait pas la différence entre le bruit et les manoeuvres (mauvaise labellisation peut être en cause) 
\newpage

\paragraph{Exemples mid ($0.5 \leq F1 < 0.7$).}
\begin{center}
    \includegraphics[width=0.98\textwidth]{figures/kalman_mid1_optim_params.png}
    \includegraphics[width=0.75\textwidth]{figures/kalman_mid1_optim_nis.png}
    \includegraphics[width=0.75\textwidth]{figures/kalman_mid1_avg_nis.png}
    \includegraphics[width=0.75\textwidth]{figures/kalman_mid1_cluster_nis.png}
\end{center}

Un exemple où notre méthode est intrinsèquement mauvaise : une manoeuvre ponctuelle subite (grande variation) augmente la précision globale du filtre, et tandis que les manoeuvres régulières de maintien à poste bien visibles sont détectées, le bruit aussi est détecté.
Les cas où le satellite ne suit pas son POL classique, avec un événement ponctuel important, fait chuter la performance de la méthode de filtrage.
\newpage
\begin{center}
...
\end{center}
Encore un exemple où la détection est bonne, mais où il faudrait plutot une méthode de seuillage par proéminence topographique.
\newpage
...
\end{center}
Toujours pareil, la proéminence topographique aurait évitée de détecter plusieurs fois la même manoeuvre. 
\newpage
\begin{center}
...
\end{center}
Excellente détection. On voit la robustesse de ce genre d'exemple même en prenant les paramètres non optimaux.
\newpage

\paragraph{Exemples perfect ($F1 = 1$).}
\begin{center}
    \includegraphics[width=0.98\textwidth]{figures/kalman_perfect1_optim_params.png}
    \includegraphics[width=0.75\textwidth]{figures/kalman_perfect1_optim_nis.png}
    \includegraphics[width=0.75\textwidth]{figures/kalman_perfect1_avg_nis.png}
    \includegraphics[width=0.75\textwidth]{figures/kalman_perfect1_cluster_nis.png}
\end{center}
Seulement 2 manoeuvres importantes, le filtre les détecte sans problème. 
\newpage
\begin{center}
...
\end{center}
Notre tolérance de 2 jours lors du matching manoeuvres vraies vs prédites (due à la labellisation imparfaite à la main) fait que les manoeuvres prédites à la suite comptent comme une seule.
\newpage
\begin{center}
...
La F1 drop dans le cas des satellites classés perfect, lorsqu'on n'applique plus les params optimaux du filtre.
\newpage
...
\begin{itemize}
    ...
    \item la robustesse au changement de params sur les objets classés good (notamment avec les params de la clusterisation) nous permet d'imaginer un déploiement éventuel de cette méthode
\end{itemize}
